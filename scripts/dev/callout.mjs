...
await p.goto('http://localhost:5173/dev/callout'); await p.waitForSelector('canvas'); await p.waitForTimeout(4000)
await p.screenshot({ path: `${OUT}/callout.png`, fullPage: true })
console.log(JSON.stringify({ canvases: await p.locator('canvas').count(), errors }))
await b.close()
