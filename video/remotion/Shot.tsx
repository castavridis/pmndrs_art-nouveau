...
import { EASINGS, type CameraKey, type ClipMeta, type Cut, type Focus } from './edit';
...
function cameraAt(keys: CameraKey[] | undefined, t: number, meta: ClipMeta, clipFrame: number) {
  const ks = keys?.length ? keys : [{ t: 0, zoom: 1 }];
  let a = ks[0]!;
  let b = ks[0]!;
  for (const k of ks) {
    if (k.t <= t) a = k;
  ...
  const span = b.t - a.t;
  const u = span > 0 ? EASINGS[b.ease ?? 'inOut'](Math.min(Math.max((t - a.t) / span, 0), 1)) : 1;
...
export function Shot({ cut, meta }: { cut: Cut; meta: ClipMeta }) {
  ...
  const cam = cameraAt(cut.camera, t, meta, clipFrame);
