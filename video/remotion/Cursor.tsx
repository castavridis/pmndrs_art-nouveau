 ...
 * ring of the brand's green where it clicks.
 */
export function Cursor({
  ...
  for (let k = Math.max(1, i - RIPPLE_FRAMES); ring && k <= i; k++) {
    ...
    if (p[2] && !meta.cursor[k - 1]![2]) {
      ...
      ripples.push({ x: rx, y: ry, age: (clipFrame - k) / RIPPLE_FRAMES });
    }
  }
  ...
  return (
    <>
      {ripples.map((r, n) => {
        const radius = size * (0.3 + 1.6 * (1 - (1 - r.age) ** 3));
        return (
          <div
            key={n}
            style={{
              ...
              height: radius * 2,
              borderRadius: '50%',
              border: `${Math.max(2, size * 0.12 * (1 - r.age))}px solid ${palette.green}`,
              opacity: 1 - r.age,
            }}
          />
        );
