  ...
  return (
    <main style={{ minHeight: '100dvh', display: 'grid', placeContent: 'center', gap: 64, padding: 64 }}>
      ...
      <section aria-label="Kinds" style={{ display: 'grid', gap: 12 }}>
        <h2 style={{ margin: 0, fontSize: 13, fontWeight: 500, opacity: 0.6 }}>All kinds (plain)</h2>
        <div style={{ display: 'grid', gap: 24 }}>
          {(Object.keys(calloutKinds) as CalloutKind[]).map((kind) => (
            <Callout key={kind} variant="plain" kind={kind} title={`${calloutKinds[kind].label} callout`}>
              <p>Body copy for a {calloutKinds[kind].label.toLowerCase()} callout. The lens carries the kind's symbol; the glass and label take its colour.</p>
            </Callout>
          ))}
        </div>
      </section>
