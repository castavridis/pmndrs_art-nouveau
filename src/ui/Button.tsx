...
const classes = ({ icon, size, className }: Common) =>
  [styles.button, icon && styles.icon, size === 'small' && styles.small, className].filter(Boolean).join(' ')
 ...
 */
export function Button({ children, icon, size, chevron, className, type = 'button', ...rest }: ButtonProps) {
  return (
